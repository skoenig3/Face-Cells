...
for i = 1:length(facewaveforms);
    if isempty(facewaveforms{i});
        remove = [remove i];
    end
end
facewaveforms(remove) = [];
nonfacewaveforms(remove) = [];
facewaveforms2(remove) = [];
nonfacewaveforms2(remove) = [];

Nov = [ones(1,length(facewaveforms)) ones(1,length(nonfacewaveforms))...
    zeros(1,length(facewaveforms2)) zeros(1,length(nonfacewaveforms2))];
Face = [ones(1,length(facewaveforms)) zeros(1,length(nonfacewaveforms))...
    ones(1,length(facewaveforms2)) zeros(1,length(nonfacewaveforms2))];


for i = 1:length(facewaveforms);
    temp1 = facewaveforms{i};
    temp2 = nonfacewaveforms{i};
    temp3 = facewaveforms{i};
    temp4 = nonfacewaveforms2{i};
    
    baseline = mean(temp2(1:200));
    temp1 = temp1-baseline;
    temp2 = temp2-baseline;
    temp3 = temp3-baseline;
    temp4 = temp4-baseline;
    
    scale = max([abs(temp1) abs(temp2) abs(temp3) abs(temp4)]);
    ...
    facewaveforms{i} = temp1;
    nonfacewaveforms{i} = temp2;
    facewaveforms2{i} = temp3;
    nonfacewaveforms2{i} = temp4;
end

data = [];
for i =1:length(facewaveforms);
    data = [data; facewaveforms{i}];
end
for i =1:length(nonfacewaveforms);
    data = [data; nonfacewaveforms{i}];
end
for i =1:length(facewaveforms2);
    data = [data; facewaveforms2{i}];
end
for i =1:length(nonfacewaveforms2);
    data = [data; nonfacewaveforms2{i}];
end
factors = [Nov' Face'];
...
for j = 1:length(performposthoc)
    ...
    if performposthoc(j)
        ...
        temptimecontrast = inversewavelettransform([tempwavecontrast 0 0],waveparams);
